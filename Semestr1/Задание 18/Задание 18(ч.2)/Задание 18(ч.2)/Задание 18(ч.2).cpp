// Задание 18(ч.2).cpp : Defines the entry point for the console application.
//
...
void SelectionSort (int input_data[], int length,CompareFunctionType compare_function_pointer)
{
	...
	{
		int minimum_index=FindMinimumIndex (input_data, length, i, length,Compare);	
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if((length%1==0)&&(length>0))
	{
		...
		cout<<"Динамический неотсортированный одномерный массив input_data."<<endl;
		OutputArray (input_data, length);		
		SelectionSort (input_data, length,compare_function_pointer);
